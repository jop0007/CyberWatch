...
public class MonitorProcesos extends Thread {
    ...
    public MonitorProcesos() {
        ...
        this.setDaemon(true);  // es un hilo daemon
    }
    
    public void iniciar() {
        Log.registrar("PROCESOS", "Iniciando simulacion de procesos");
        
        // cargo la blacklist del archivo
        cargarBlacklist();
    ...
    private void cargarBlacklist() {
        File archivo = new File("blacklist.txt");
        if (!archivo.exists()) {
        ...
        try {
            ...
            if (blacklist.size() > 0) {
                Log.registrar("PROCESOS", "Lista negra cargada: " + blacklist.size() + " procesos");
                // muestro los procesos en la lista negra
                for (int i = 0; i < blacklist.size(); i++) {
                    Log.registrar("PROCESOS", "  - " + blacklist.get(i));
                }
            }
    ...
    private void generarProcesos(int cantidad) {
        ...
        for (int i = 0; i < cantidad; i++) {
            ...
            Proceso p = new Proceso(nombres[indice], cpus[indice]);
            procesos.add(p);
    ...
    private void detectarAnomalias() {
        ...
        for (int i = 0; i < procesos.size(); i++) {
            ...
            // DETECCION 1: esta en la blacklist?
            ...
            // DETECCION 2: CPU alta y persistente?
            if (p.getCpu() > 80 && p.getTiempoVivo() > 15) {
                ...
                // lo añado a la blacklist
                añadirABlacklist(p.getNombre());
                procesosEliminados++;
            }
        }
    }
    ...
    private boolean estaEnBlacklist(String nombre) {
        for (int i = 0; i < blacklist.size(); i++) {
            if (blacklist.get(i).equals(nombre)) {
                return true;
            }
        }
        return false;
    ...
    private void añadirABlacklist(String nombre) {
        ...
        try {
            BufferedWriter writer = new BufferedWriter(new FileWriter("blacklist.txt", true));
            writer.write(nombre);
            writer.newLine();
            writer.close();
